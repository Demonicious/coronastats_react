...
const getCountrySpecificLines = async (name) => {
  name = name.toLowerCase().replace('.', '').replace(/\s+/g, '').trim();
  let res = await fetch(`https://coronastats.co/builds/countries/${name}.json`);
  let data = await res.json();
  return data;
}

const constructChart = async (name, props, setChartData) => {
  getCountrySpecificLines(name).then(lines => {
...
export const CountryDetailsBackdrop = (props) => {
    const [chartData, setChartData] = useState(null);
    const { country, setCountryItem } = props;
    let received = false;
    if(country && typeof country !== 'undefined') received = true;
    let mild_rate = 0;
    let critical_rate = 0;
    if(received) {
      constructChart(country.name, props, setChartData);
              ...
              <div id="countryDataChartDraw">
                {
                        ...
                        <div style={{width: '100%', height: '100%'}} id="countryDataChart">
                          <Line data={chartData[0]} options={chartData[1]} />
                        </div>
                        : 
